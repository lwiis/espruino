var doMorseCodeFlag = true;
var doTemperatureFlag = true;
var isDoingTemperatureFlag = false;
...
function resetCustom() {
  ...
  LED3.set();
  buttonPressed = false;
  doMorseCodeFlag = true;
  doTemperatureFlag = true;
  morseInProgress = "";
  history = Int16Array(total).fill(E.getTemperature());
  if(isDoingTemperatureFlag) {
    clearInterval(temperatureInterval);
    isDoingTemperatureFlag = false;
  }
  LED1.reset();
  LED2.reset();
  LED3.reset();
...
//TODO only start advertising when button clicked?
...
function advertiseTemperature() {
  ...
  console.log('Temperature: ' + avg);
  
  NRF.setAdvertising({}, {
    manufacturer: 0x590,
    manufacturerData: [Puck.getBatteryPercentage(), avg, buttonPressed]
  });
  
  if(isDoingTemperatureFlag && avg>=32) {
    doMorseCodeFlag = false;
    clearInterval(temperatureInterval);
    LED2.set();
  }
}
...
var watchId = setWatch(function() {
  ...
  if(!buttonPressed) {
    buttonPressed = true;
    console.log('set buttonPressed flag');
  }
  if(doMorseCodeFlag) {
  ...
  if(doTemperatureFlag && !isDoingTemperatureFlag) {
    temperatureInterval = setInterval(advertiseTemperature, 2000);
    advertiseTemperature();
    isDoingTemperatureFlag = true;
  }
